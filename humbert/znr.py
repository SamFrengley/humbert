...
surfaces in the birational equivalence class of the surface Z_(N,r). We also
provide some functionality which allows a user to automatically generate
pictures of cusps on Z_(N,r), together with their intersection behaviour with
...
- Ztil(N, r)
.. todo::
    Define a ZNr_o class. Currently this is all bundled into the WNr.py module
    in a way which you can't get at the formulae just for Z^o(N, r).

Examples
--------
Some basic usage. This first example shows how to use the functionality which
follows simply by looking up formulae in Kani--Schanz [2]_ which should return
Table 1 from [2]_ (cf. [1]_ though note there are mistakes as pointed out by
[2]_).

>>> from humbert.znr import Ztil
>>> Z = Ztil(12, 1)                                         # ~Z(12, 1)
>>> Z.kodaira_dim()                                         # ell K3
0
>>> print(Z)                                                # display invariants
The surface ~Z_(12,1)
κ     = 0
p_g   = 1
K^2   = -16
...
The following example shows how to use the code to generate the intersection
multiplicities in Prop. 2.5 of [3]_ and of p.98 in [2]_.

>>> from humbert.znr import ZtilCusp
>>> C = ZtilCusp(21, 5, 21, 5)                              # E_(∞,21,5) on Z_(21,5)
>>> C.ctd_frac                                              # gives self-ints
[5, 2, 2, 2, 2]
>>> C.mults                                                 # multiplicities
([21, 5, 4, 3, 2, 1, 0], [0, 1, 5, 9, 13, 17, 21])

The continued fraction means that the component is a (-5,-2,-2,-2,-2)-chain
capped off by the curves C_(∞, i). The multiplicities are the multiplicities in
j*(∞) and (j')*(∞) -- including C_(∞,1) and C_(∞,2) at the ends.

The final example focusses on the "picture drawing" capabilities.

>>> from humbert.znr import Ztil
>>> Z = Ztil(21, 5)
>>> _ = Z.sketch_cusps(disp=True)                           # Doesn't make pdf
>>> _ = Z.sketch_cusps(tex=True, compile_tex=True, open_pdf=True)

You should compare this to [1]_ Section 11.4 "The case (21, 5)". It also shows
a TODO in the implementation which is to show intersections which occur at the
divisors C_{∞,i} -- the components of F_41 meet C_{∞,i} and the first
component of E_{∞,21,20} by the projection formula (now mentally fill in
Figure 19). 

Notes
-----
One should compare the output of the functions in this file with the tables in
the back of [2]_. We did some quite extensive testing to make sure they match
up.

If I ever get time I'd like to make these pictures include intersections of HZ
divisors which occur at CM points, and to include E_(2,1) and E_(3,1). The
lofty goal of course is to make this code work in the generality of Hilbert
modular surfaces of arbitrary discriminant and level structure. This is of
course some way off.
...
.. [2] C.F. Hermann, 'Modulflachen quadratischer Diskriminante', Manuscripta
   math. 72 (1991), no. 1, 95-110.
.. [3] E. Kani and W. Schanz, 'Modular diagonal quotient surfaces', Math. Z. 277
   (1998), no. 2, 337-366.

"""

################################################################################
# A matplotlib global configuration
...
################################################################################
# matplotlib configurations for figures
import matplotlib
...
################################################################################
# The class of the Hirzebruch--Zagier divisor F_m on ~Z_(N,r)

def comp_mults(N, d, q):
    """The multiplicities of the components of a cusp resolution on ~Z_(N,r)

    This is a helper function for both F_m and for the cusps on ~Z_(N,r) which
    computes the multiplicites in j*(∞) and (j')*(∞) respecitively.
...
def err(N, m):
    """The correction term 𝖊 in the formula for K.F_m on ~Z_(N,r)

    In the paper this is denoted 𝖊 and is the last term in the formula for the
    intersection of a canonical divisor and a HZ divisor F_(m,λ) on ~Z_(N,r).
...
class F_m:
    ...
    def make_cusps(self):
        ...
        This method initialises and computes the arguments self.cusps,
        self.comps, and self.int_mults.        
        ...
        for i in range(0, len(self.cusps)):
            ...
            while not found:
                ...
                if st[0] > -0.01 and st[1] > -0.01:  # check are positive
                    ...
                    ):  # check that they are ints (is there a better way?)
                        found = True
                        if st_int[1] == 0:
                            kk = [k]
                            inters = [st_int[0]]
...
################################################################################
# The class of the cusps on ~Z_(N,r)

def format_table_same_col_widths(*ls, sep="|"):
    """Aligns lists in a table with columns of the same widths.

    This is a helper function for outputting what the interesctions in a cusp
...
class ZtilCusp:
    ...
    def intersection_table(self):
        """Self intersections and multiplicities of divisors
    ...
    def sketch_cqr_odd_part(self, part, E_labels=False, SI=True, eps=0.1):
        """Subroutine for `sketch_cyc_quot_res_odd`. Sketches the first or
        second part of E_(∞,d,q) when the continued fraction has odd length.
    ...
    def sketch_Fg(self, FPs=True, non_fixed=True, eps=0.1):
        ...
        do better to refer to [1]_ Proposition 6.13 (cf. [2]_ when N = p).

        Possible improvement: Most of the pain in the implementation here is
        just getting the picutres to look "visually appealing" -- hence all the
        tweaked parameters etc. Doing this in a "better" way is almost certainly
        nessicary for more generality (say Hilbert modular surfaces in general).
...
class Ztil:
    ...
    def K_Fm(self, m):
        ...
        ]:  # should change this is a O(N) implementation
    ...
    def sketch_cusps(
        ...
        if tex:
            init_dir_name = "./figs/"
            dir_name = init_dir_name + f"Z{N}-{r}/"
            f_names = []
            caps = []
            # make sure we're outputting the tex into a directory we know exists
            try:
                os.mkdir(init_dir_name)
