...
This module includes the helper functions which we need to deal with modular
curves in the context of the paper.
...
def RH_genus(mu, e_2, e_3, e_inf):
    ...
    Notes
    -----
    This formula is Theorem 3.1.1 of [1]_.
...
def get_rat_X0_plus():
    ...
    Note
    ----
    Implemented as a hardcoded list after p.178 of [3]_ (see also [1]_ and [2]_).

    References
    ----------
    .. [1] H. Cohn, 'A numerical survey of the reduction of modular curve genus
       by Fricke’s involutions', Number theory (New York, 1989/1990), Springer,
       New York, 1991, pp. 85–104.
    .. [2] M. Furumoto and Y. Hasegawa, 'Hyperelliptic quotients of modular
       curves X_0(N)', Tokyo J. Math. 22 (1999), no. 1, 105–125.
    .. [3] H. Helling, 'Note uber das Geschlecht gewisser arithemetischer
       Gruppen', Math. Ann. 205 (1973), 173-179.
...
def fricke_sort(N, cusps, widths=[]):
    ...
    This is a helper for class X_0.
    ...
    -------
    list
        The list of cusps, ordered so that the Fricke involution acts on the set
        of cusps by reversing this list.
    list
        The widths corresponding to the cusps in the first return.
...
class X_0:
    ...
    cusps : list
        A list of tuples (d,x) where d|N and x runs over reps for
        ℤ/gcd(N,N/d)ℤ. The order of this list is chosen so that the Fricke
        involution acts as cusps.reverse().
    widths : list
        A list of cusp widths corresponding directly to the cusps.

    Note
    ----
    The formulae here are well known. A reference is e.g., Prop. 1.43 of [2]_.
    The widths of the cusps are slightly less obvious but it is an easy exercise
    from the definition in p. 59 of [1]_.

    References
    ----------
    .. [1] F. Diamond and J. Shurman, 'A first course in modular forms',
       Graduate Texts in Mathematics, vol. 228, Springer-Verlag, New York, 2005.
    .. [2] G. Shimura, 'Introduction to the Arithmetic Theory of Automorphic
       Functions', Princeton University Press, Princeton, 1971.
    ...
    def make_cusps(self):
        ...
        (d, x) |-> [x/d]. It is easy (see [1]_) to show that the width of such a
        cusp is equal to the least integer 0 < w < N such that
        w * d^2 == 0 (N).

        References
        ----------
        .. [1] TODO, FIND REFERENCE?
...
class X_w:
    """Class of the modular curve X_w for odd N (note the lack of +)
    ...
    def genus(self):
        ...
        -------
        int
            The genus of the modular curve X_w
...
class Xg_plus:
    ...
    Note that this is not strictly "the modular curve X_g^+" -- though such
    functionality would be desirable. Instead this class essentially
    encapsulates a list of curves X_g^+ where g ranges over the conjugacy
    classes of elements of GL_2(ℤ/Nℤ) which satisify g^2 = -det(g). When N is
    odd this is [ X_w^+ ].
    ...
    def init_ramification(self):
        """Computes the rational numbers μ+, e_2+, e_3+, e_∞+ (these are
        integers when N != 1).
    ...
    def genus(self):
        ...
        Notes
        -----
        This is implemented as a lookup to [1]_ Corollaries 7.2-7.4.
